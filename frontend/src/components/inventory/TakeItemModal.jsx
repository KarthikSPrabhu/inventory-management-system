        ...
        <div className="relative w-full max-w-md bg-white border border-slate-200 rounded-2xl shadow-2xl overflow-hidden z-10 animate-fadeIn max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-slate-100">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h3 className="text-sm font-black text-slate-900 tracking-tight uppercase">TAKE INVENTORY</h3>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-600 block">
                Quantity to Take <span className="text-indigo-600">*</span>
              </label>
              <input
                type="number"
                min="1"
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center">
                <label className="text-xs font-bold text-slate-600 block">
                  Project <span className="text-indigo-600">*</span>
                </label>
                
                <button
                  type="button"
                  onClick={() => setIsCreateProjectOpen(true)}
                  className="text-[11px] font-extrabold text-indigo-600 hover:text-indigo-300 transition-colors flex items-center gap-1"
                >
              ...
              {loadingSuggestions ? (
                <div className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs text-slate-500 animate-pulse">
                  Loading project suggestions...
                </div>
              ) : suggestions.length === 0 ? (
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-center space-y-2">
                  <p className="text-xs text-slate-500 font-medium">No active projects found.</p>
                  <button
                    type="button"
                    onClick={() => setIsCreateProjectOpen(true)}
                    className="inline-flex items-center gap-1 bg-indigo-600 hover:bg-indigo-50 text-slate-900 font-bold text-xs px-3 py-1.5 rounded-lg transition-all"
                  >
                    + Create Project Now
                  </button>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <option value="CREATE_NEW" className="text-indigo-600 font-bold">
                      + Create New Project...
                    </option>
                  </select>
                  ...
                  {selectedProject && (
                    <div className="flex items-center gap-1.5 px-1">
                      {selectedProject.usedBefore ? (
                        <span className="inline-flex items-center gap-1 text-[11px] font-bold text-amber-600 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-md">
