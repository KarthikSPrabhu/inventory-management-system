    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 pb-5">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">Track components used across your hardware and software projects.</p>
        </div>
      ...
      {error && (
        <div className="bg-rose-50 border border-rose-200 p-5 rounded-2xl flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h5 className="text-sm font-bold text-slate-900">Error</h5>
              <p className="text-xs text-slate-500 mt-0.5">{error}</p>
            </div>
          </div>
          <button
            onClick={fetchProjectsList}
            className="bg-rose-50 hover:bg-rose-50 text-rose-600 hover:text-slate-900 border border-rose-200 font-bold text-xs px-4 py-2 rounded-xl transition-all"
          >
            Try Again
          </button>
        </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          <div className="bg-white border border-slate-200 rounded-2xl h-44 animate-pulse" />
          <div className="bg-white border border-slate-200 rounded-2xl h-44 animate-pulse" />
          <div className="bg-white border border-slate-200 rounded-2xl h-44 animate-pulse" />
        </div>
      ) : projects.length === 0 ? (
        <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center flex flex-col items-center justify-center space-y-4">
          <div className="h-14 w-14 rounded-2xl bg-indigo-50 border border-indigo-500/20 flex items-center justify-center text-indigo-600">
            <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
            </svg>
          </div>
          <h4 className="text-base font-extrabold text-slate-900">No projects created yet</h4>
          <p className="text-xs text-slate-500 max-w-sm">
            Create a project to begin taking inventory items and tracking component usage per project.
          </p>
          <button
            onClick={() => setIsCreateModalOpen(true)}
            className="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-50 text-slate-900 font-extrabold text-xs px-5 py-2.5 rounded-xl transition-all shadow-md"
          >
            + Create Project
          </button>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-10 text-center">
          <p className="text-xs text-slate-500">No projects match "{searchQuery}".</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredProjects.map((proj) => {
            ...
            if (proj.status === 'archived') statusBadge = 'bg-slate-50 text-slate-500 border-slate-500/25';
            ...
            return (
              <Link
                ...
                className="bg-white border border-slate-200 hover:border-indigo-500/60 rounded-2xl p-5 flex flex-col justify-between shadow-md hover:shadow-lg transition-all group cursor-pointer"
              >
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-lg border border-slate-200 hover:bg-rose-50 text-slate-500 hover:text-rose-600 hover:border-rose-300 transition-colors"
                      >
                        {isDeletingThis ? (
                          <svg className="w-3.5 h-3.5 animate-spin text-rose-600" fill="none" viewBox="0 0 24 24">
                ...
                <div className="pt-4 mt-4 border-t border-slate-200 flex items-center justify-between">
                  <span className="text-xs font-semibold text-slate-500">
                    <strong className="text-indigo-600 font-mono">{stats.differentItemsCount}</strong> items &bull; <strong className="text-emerald-600 font-mono">{stats.totalUnitsUsed}</strong> units
                  </span>
                </div>
              </Link>
            );
